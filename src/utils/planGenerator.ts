import { DailyPlan } from "@/types/fitness";
...
const getMealsByGoal = (goal: string) => {
...
export const generateWorkoutPlans = (profile: Profile) => {
  const goal = profile.fitness_goal || "maintenance";
  const goalKey = goal as keyof typeof exerciseTemplates;

  const weeklyPlan = [
    { day: "Monday", exercises: exerciseTemplates[goalKey] || exerciseTemplates.maintenance },
    { day: "Tuesday", exercises: legExercises[goalKey] || legExercises.maintenance },
    { day: "Wednesday", exercises: backExercises[goalKey] || backExercises.maintenance },
    { day: "Thursday", exercises: [{ id: "e13", name: "Rest Day", sets: 0, reps: 0, description: "Active recovery - light stretching or walk", completed: false }] },
    { day: "Friday", exercises: exerciseTemplates[goalKey] || exerciseTemplates.maintenance },
    { day: "Saturday", exercises: legExercises[goalKey] || legExercises.maintenance },
    { day: "Sunday", exercises: [{ id: "e22", name: "Rest Day", sets: 0, reps: 0, description: "Full recovery - enjoy your day!", completed: false }] },
  ];

  return weeklyPlan;
};

export const generateMealPlans = (profile: Profile) => {
  const goal = profile.fitness_goal || "maintenance";
  const meals = getMealsByGoal(goal);

  return [
    { day: "Monday", meals: meals.monday },
    { day: "Tuesday", meals: meals.tuesday },
    { day: "Wednesday", meals: meals.wednesday },
    { day: "Thursday", meals: meals.thursday },
    { day: "Friday", meals: meals.friday },
    { day: "Saturday", meals: meals.saturday },
    { day: "Sunday", meals: meals.sunday },
  ];
};

export const getCalorieTarget = (profile: Profile) => {
  ...
  switch (activityLevel) {
    ...
    case "light": activityMultiplier = 1.1; break;
    case "moderate": activityMultiplier = 1.2; break;
    case "active": activityMultiplier = 1.35; break;
  }

  let goalMultiplier = 1;
  switch (goal) {
    case "weight_loss": goalMultiplier = 0.85; break;
    case "muscle_gain": goalMultiplier = 1.15; break;
    case "maintenance": goalMultiplier = 1; break;
  }

  return Math.round(baseCalories * activityMultiplier * goalMultiplier);
};
